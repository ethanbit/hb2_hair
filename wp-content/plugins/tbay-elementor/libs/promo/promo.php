<?php
/**
 * This file belongs to the Thembay Elementor.
 *
...
if( apply_filters( 'tbay_plugin_notice', true ) ){
	...
	if( ! function_exists( 'tbay_plugin_promo_notices' ) ){ 
		function tbay_plugin_promo_notices(){
			$xml                        = apply_filters( 'tbay_elementor_plugin_promo_xml_url', 'https://bitbucket.org/devthembay/update-plugin/raw/master/promo/tbay-framework-promo.xml' );
		    ...
		    if( false === $remote_data || apply_filters( 'tbay_plugin_force_regenerate_promo_transient', false ) || 'yes' == $regenerate_promo_transient ){
	...
	if( ! function_exists( 'tbay_plugin_notice_dismiss' ) ){
		function tbay_plugin_notice_dismiss(){
		    wp_enqueue_script( 'tbay-elementor-promo', TBAY_ELEMENTOR_URL . 'assets/promo.js', array( 'jquery'  ), '1.0.0', true );
		    wp_enqueue_style( 'tbay-elementor-promo', TBAY_ELEMENTOR_URL . 'assets/promo.css', '1.0.0', true );
		    wp_enqueue_script( 'tbay-elementor-promo' );
		}
	}
