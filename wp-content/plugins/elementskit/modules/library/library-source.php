...
class Library_Source extends Source_Base {
	...
	public static function get_license(){
		...
		return [
			'key' => \ElementsKit\Libs\Framework\Classes\Utils::instance()->get_option('license_key'),
	...
	public static function request_template_data( $template_id ) {
		...
		global $wpdb;
		$template_count = $wpdb->get_var( "SELECT COUNT(*) FROM ".$wpdb->prefix."elementor_kits_template_data WHERE template_id = $template_id" );

		if($template_count){
			$wpdb->update( 
				$wpdb->prefix.'elementor_kits_template_data', 
				array( 
					'template_data' => $data,
		...
		// if(!$template_count){
		// 	$data = '{"version":"0.4","title":"Oop! Error","type":"page","content":[],"isInner":false,"elType":"section"}]}';
		// }else{
		// 	$templateData = $wpdb->get_row( "SELECT template_data FROM ".$wpdb->prefix."elementor_kits_template_data WHERE template_id = $template_id" );
